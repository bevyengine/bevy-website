...
11. To make sure your web-based files (html, markdown, css) are formatted correctly run the command `markdownlint -f -c .github/linters/.markdown-lint.yml . && djlint` in the root directory of your local Bevy website repository. This will format markdown files and tell you the issues in HTML files. In order to run the command you should install `markdownlint-cli` and `djlint`. See: <https://github.com/igorshubovych/markdownlint-cli> and <https://www.djlint.com/docs/getting-started/> Note that the CI also includes `editorconfigchecker` but there isn't an easy way to run this manually, so you should instead enable `editorconfig` in your preferred editor or rely on CI.
