...
10. To validate if local code changes are compiling and have the right formatting you can go into the `code-validation` folder using the `cd` command. After that you can test your code using `cargo test` and check the formatting of the code using `cargo +nightly fmt --check`. Note that you will have to fix formatting issues manually because running `cargo +nightly fmt` won't fix any formatting issues for you. The reason for this is that it would only format the code inside of the `code-validation/lib.rs` file, but not inside the corresponding markdown files.
