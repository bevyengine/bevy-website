...
Note that we have used `.chain()` on the two systems. This is because we want them to to run in exactly the order they're listed in the code: with `update_people` occurring before `greet_people`.
