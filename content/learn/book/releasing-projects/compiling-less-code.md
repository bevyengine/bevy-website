...
Bevy, as a game engine, offers a wide variety of functionality out-of-the-box.
Almost no project will take advantage of all of the features at the same time,
bloating both your final binary and your compilation times.
However, the set of features that are used will vary between projects so we should not simply cut them!

To get the best of both worlds, we need to allow users to customize which parts of Bevy they pull in.
...
In Rust, you can designate features as "default", causing them to be enabled for users who add this crate to their project.
...
for both starting projects and running examples.
As a result, they enable most of Bevy's functionality, and the resulting setup is not particularly lightweight.
...
To turn off default features for , you instead need to configure your `Cargo.toml` like so:
...
- `2d`: for 2D games
- `3d`: for 3D games
- `ui`: to use Bevy's built in UI solutions
- `audio`: to use Bevy's audio solution
- `dev`: various tools for project development
  - this flag should not be enabled for your released builds

Disabling default features and using these feature collections allows you to quickly cut out large swaths of the engine
that you can be confident that your project will not need, without spending a great deal of time digging into and updating more granular feature flags.
...
This process can be quite time-consuming and frustrating (especially when updating Bevy versions), and we recommend only doing so with good reason.
...
2. Start with a fairly permissive set of features, construct a working project and then pare them down. Replace feature collections with their corresponding set of features, then attempt to delete features one-at-a-time and see what breaks.
...
Unlike in many other languages, Rust projects can have multiple copies of the same dependency that use different versions.
...
Resolving duplicate dependencies can be quite challenging: asking nicely upstream to bump their dependency is often effective,
but coordinating releases is quite challenging!
...
These costs scale substantially the more you diverge from upstream, or if you wish to pull in new features and bug fixes.
