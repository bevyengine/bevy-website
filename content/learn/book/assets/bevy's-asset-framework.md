...
This is handled by the [`AssetServer`], which handles the complex process of turning the path of an asset into usable data in memory.
...
[`Asset`]: https://docs.rs/bevy/latest/bevy/prelude/trait.Asset.html
[`Mesh3d`]: https://docs.rs/bevy/latest/bevy/mesh/struct.Mesh3d.html
...
1. Call load on your asset: `AssetServer::load("bevy_bird.png")`, for example.
...
Trying to immediately use an asset handle can result in an object not rendering, a sound not playing, etc.
You can call `AssetServer::load("bevy_bird.png")` and receive a [`Handle<Image>`] that can be used to create a [`Sprite`], but you might not immediately see the `"bevy_bird.png"` image in your game.
...
Both approaches will change what your sprite looks like, and both are valid to do, but the effects differ in two important ways:

- Mutating the _handle_ changes the [`Image`] asset your sprite entity is pointing to.
  - This only affects the entity you specify.
  - You can think of it as replacing one `&` reference with a different `&` reference.
- Mutating the _asset_ changes the underlying data that the handle is pointing to.
...
Because of this, some [`RenderAsset`]s may be configured to unload their CPU data, saving significant amounts of RAM.

This can present issues when attempting to mutate asset data, as the actual data (like the pixel data of an image) is moved to the GPU, even though the asset remains loaded.
